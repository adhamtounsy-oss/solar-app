...
// Cairo soiling loss by month (dust/Khamsin March-May are worst) 
...
const SIGMA_MODEL = 0.030;
const SIGMA_TOT   = Math.sqrt(SIGMA_IRR*SIGMA_IRR + SIGMA_MODEL*SIGMA_MODEL); // ≈ 0.0583

/**
 * Bi-linear degradation — IEA-PVPS T13-2021 two-phase field evidence
 * Years 0-2 (first 3 cal. years): rate × 1.3 accelerated; Year 3+: steady-state
...
// EgyptERA tiered tariff blocks (kWh/month thresholds → EGP/kWh) 
// EgyptERA residential tariff — effective April 2026 
// Source: Ministry of Electricity internal circular (Masrawi, 4 April 2026) 
// Tiers 1–6 unchanged from August 2024; Tier 7 raised from E£2.23 → E£2.58 (+16%) 
// Coded meters (
const EGYPT_TARIFF_TIERS = [ 
...
function tieredMonthlySaving(consumedKwh, savedKwh, escFactor) { 
...
// Smart meter CSV parser — returns Float32Array(8760) or null 
...
function parsePVGISJson(data) { 
  ...
  // D1/D3: additional fields for Faiman thermal model and hourly IAM
...
["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]; 
  ...
  const monthly = DAYS.map((days, mi) => { 
    ...
    return { 
      ...
      gPoaAvg: parseFloat((sumG/count).toFixed(1)), // mean hourly W/m² for the month 
      days, 
    }; 
  }); 
  // Kimber soiling: extract daily precipitation sums (RR field, mm/hr in PVGIS ERA5)
...
function parseONDFile(text) { 
  ...
  // Extract efficiency curve points if present 
  const effCurve = []; 
  lines.forEach(l => { 
  ...
  return { 
    ...
    eta, thd: 3.0, 
    effCurve,  // E19: full efficiency curve for part-load correction 
...
async function fetchPVGIS(lat, lon, tilt, azimuth) { 
  ...
  // PVGIS ERA5 hourly TMY — direct browser fetch 
  // The EC JRC API supports CORS (Access-Control-Allow-Origin: *) 
  // Previous proxy approach via Claude API failed because 8,760 rows of JSON 
  // far exceeds the tool output window — direct fetch is correct here. 
  // 
  // Two URLs tried in sequence: 
  //   1. v5_2 seriescalc (preferred — returns G_i corrected plane-of-array irradiance) 
  //   2. v5_2 tmy endpoint (fallback — older format but more stable) 
  const pvgisUrl = 
...
parameters"); 
  ...
  // This is best-effort — errors are swallowed; main fetch already succeeded
  let horizonProfile = [];
  try {
    const hUrl = `https://re.jrc.ec.europa.eu/api/v5_2/printhorizon?lat=${lat}&lon=${lon}&outputformat=json`;
    const hRes = await fetch(hUrl, {
      headers: { "Accept": "application/json" },
      signal: (new AbortController()).signal  // no timeout — best effort
    });
    if (hRes.ok) {
      const hJson = await hRes.json();
      // PVGIS printhorizon returns {outputs:{horizon:[{A:azimuth, H_hor:elevation}]}}
...
// Source: Faiman D. (2008), Progress in Photovoltaics 16(4):307-315
function cellTempFaiman(G, Ta, ws, U0, U1) {
  if (G <= 0) return Ta;
...
function lowIrradianceFactor(G_wm2, panel) {
...
function solarCosIncidence(hr, lat_deg, tilt_deg, az_deg) {
...
// Step 1: Fit reference parameters at STC (G_ref=1000 W/m², Tc_ref=25°C)
// Inputs: panel.isc, voc, imp, vmp, wp, ns (optional)
function fitOneDiodeParams(panel) {
  const Isc = panel.isc, Voc = panel.voc, Impp = panel.imp, Vmpp = panel.vmp;
  if (!Isc || !Voc || !Impp || !Vmpp || Vmpp >= Voc || Impp >= Isc) return null;
  // Ns: number of cells in series — estimate from Voc (modern c-Si cell Voc ≈ 0.685V)
  const Ns  = panel.ns || Math.max(36, Math.round(Voc / 0.685));
  // n=1.2 (PVsyst default for c-Si — captures both junction and recombination effects)
  const Vth = 0.025693;             // kT/q at 298.15 K [V]
  const a   = 1.2 * Ns * Vth;      // diode thermal voltage at STC [V]
  // Simplified De Soto: assume Rsh >> 1 for initial I0, IL estimates
  const I0_init = Isc * Math.exp(-Voc / a);
  if (I0_init <= 0) return null;
  // Rs from MPP power constraint (Villalva 2009, Eq. 15)
  const lnArg = (Isc - Impp) / I0_init;
  if (lnArg <= 0) return null;
  const Rs  = Math.max(0, (a * Math.log(lnArg) - Vmpp) / Impp);
  // Rsh from two-point (Voc + MPP) method — more stable for high fill-factor panels
  // Derived from diode equation at both points: Rsh = (Voc - Vmpp - Impp*Rs) / (I0*(e^x1-e^x2) - Impp)
  ...
  const Rsh = (rshDenom > 0.01 * Isc)
    ...
    : 500;   // fallback for numerically ill-conditioned high-FF panels
  // Refine IL, I0 with finite Rsh
...
// Step 2: Translate reference params to operating conditions (G W/m², Tc_C °C)
// De Soto (2006) translation equations — Eqs. 11–15
function translateOneDiode(ref, G, Tc_C) {
  if (!ref || G <= 0) return null;
  const Tc   = Tc_C + 273.15;        // °C → K
  const Tref = 298.15;               // K (25°C)
  // Photocurrent: linear in G, slight T dependence (alphaIsc default +0.05%/°C for c-Si)
  const alphaIsc = (ref.alphaIsc || 0.05) / 100;
  const IL  = ref.IL * (G / 1000) * (1 + alphaIsc * (Tc_C - 25));
  // Saturation current: T^3 × exp(Eg/(n·kB) × (1/Tref − 1/T))
  // Eg_nkB = Eg_Si [eV] × Ns × Tref / a_ref  [Kelvin]
  ...
  // Thermal voltage scales with absolute temperature
  const a   = ref.a * (Tc / Tref);
  // Rsh scales inversely with G (more recombination at low light)
  const Rsh = ref.Rsh * (1000 / G);
  const Rs  = ref.Rs;
  return { IL, I0, Rs, Rsh, a };
}

// Step 3: Solve for Pmpp/Wp_stc using golden-section search on P(V)
// Returns normalised power ≈ (G/1000) × efficiency_correction; ≈ 1.0 at STC
function solveMPP_norm(tr, Wp_stc) {
  if (!tr || tr.IL <= 0 || tr.I0 <= 0) return 0;
  const { IL, I0, Rs, Rsh, a } = tr;
  // Newton-Raphson solver for I(V): f(I) = I - IL + I0*(exp((V+I·Rs)/a)-1) + (V+I·Rs)/Rsh
  function solveI(V) {
  ...
  // Estimate open-circuit voltage at operating conditions
  const Voc_op = Math.min(a * Math.log(Math.max(1, IL / I0)), a * 80);
  // Golden-section search for Vmpp over [0.5·Voc_op, 0.98·Voc_op]
...
function kimberSoiling(precipDaily, sRate, threshold, afterRain, initSoil) {
  const sr   = sRate    || 0.0015;   // fraction/day accumulation rate (Cairo = 0.15%/day)
  const thr  = threshold || 0.5;     // mm/day cleaning threshold (light rain cleans panels)
  const ar   = afterRain || 0.005;   // residual soiling after rain event (0.5%)
  const init = initSoil  || 0.005;   // initial soiling (0.5%)
  const DAYS = [31,28,31,30,31,30,31,31,30,31,30,31];
...
// D5: One-diode (De Soto) module model — replaces linear G×η+Faiman+LI when gpoa available
// E1: clipping — gen capped at inverter.acKW per hour
// E3: split charge/discharge efficiency (η applied per timestep)
// E12: meterDemands overrides synthetic profile when smart meter is loaded
// E17: zeroExport — curtails instead of exporting when mode is active 
function runHourlyDispatch(hourlyGenKwp, actKwp, etaSysFixed, soilingByMonth, 
  ...
  // B5: Battery temperature derating — LFP loses capacity at low T (IEC 62619) 
  // Approximate: −0.6% per °C below 25°C, based on manufacturer discharge curves 
  // Use monthly average ambient as proxy for battery enclosure temp 
  // We'll apply a conservative fixed winter derating for outdoor installations in Cairo 
  // (Cairo January avg 14°C → ~6.6% capacity reduction for outdoor battery) 
...
0; 
  ...
  return { 
    ...
    eveningDeficits, sohFactor, usableCap, 
...
const DEF = { 
  ...
  // tAmbMin: IEC 62548-1:2023 requires record/design minimum, not monthly average.
  // Cairo record cold: −2°C (Jan 2008). With 20 panels × LONGi 580W, Voc crosses
  // the 1100V Vdc_max at ~6°C ambient. Setting 5°C forces nMax=19 → nSel=19 safely.
...
  lenStringM:25, lenFeederM:15, lenBatteryM:3, lenACM:20, 
  ...
  netMeteringRate:0.50,     // EGP/kWh export tariff (net-metering scheme)
  // TOU dispatch
  touEnabled:false, touPeakStart:17, touPeakEnd:22, gridChargeEnabled:false,
  // Optimizer / MLPE analysis
  costPerOptimizerUSD:30, shadingLossFraction:0.05,
  // i18n
  lang:"en",
  discountRate:12,          // E11: NPV discount rate % 
  albedo:0.20,              // E8:  ground reflectance (0.20=concrete, 0.35=white membrane) 
  bifacialFactor:0.70,      // E8:  rear/front cell efficiency ratio (typical LFP: 0.65-0.75) 
  // E13: near-shading obstacles (array of {h, d, az} — height m, distance m, azimuth °) 
  // B4: Monthly soiling profile — editable per project (default = Cairo) 
  // Values are fraction lost (0.02 = 2% soiling loss that month) 
  soilProfile:[0.02,0.02,0.09,0.11,0.08,0.02,0.02,0.02,0.02,0.03,0.02,0.02], 
  obstacles:[], 
  // E14: horizon profile [{az, elev}] — solar elevation blocked at each azimuth 
  horizonProfile:[], 
  // E16: sub-arrays for multi-orientation [{tilt, az, panels, label}] 
  subArrays:[], 
...
//                  so the profile fractions set the shape, acHrs sets the magnitude. 
function computeLoadProfile(inp, billScale, seasonalAcScale) { 
...
// Compute seasonal AC scale for a given month index 
// Fix 5: acHrsSummer / acHrsWinter directly control AC hours per day. 
// Profile-implied AC hours = sum(acFr[w] * WIN_HRS[w]) — the profile sets shape. 
// seasonalAcScale = actualDailyHours / profileImpliedHours 
function seasonalAcScale(inp, monthIdx) { 
...
//   eta_reflect removed — subsumed by hourly IAM correction in calcEngine
//   eta_quality removed — over-conservative for premium mono; no site data support
//   eta_shading removed — site-specific; constant 2% biases all zero-shade rooftops
// Upper clamp raised 0.85 → 0.92 (old cap silently masked winter values).
function computeEtaSys(panel, tAmb) {
...
function runOpt(inp, panelLib, invLib, batLib, selInv, selBat) { 
...
const cardS = color => ({background:C.card,borderRadius:10,overflow:"hidden", 
...
function Bar({val,max,color,width=70}) { 
